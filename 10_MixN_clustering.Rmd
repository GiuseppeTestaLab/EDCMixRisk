...
# we cluster DEGs by foldchange in the chronic datasets
lfcs <- prepLFC4clust2(assay(se)[degs2,],groups=paste(se$Dataset,se$EXPO))
lfcs2 <- lfcs[,grep("T3|BPA",colnames(lfcs),invert=TRUE)]
set.seed(1234)
cl <- clusterWrapper(lfcs2, 2:15, nstart=3)
rowData(se)$cluster <- factor(cl[["8"]]$cluster[row.names(se)])
cg <- split(names(cl[["8"]]$cluster), cl[["8"]]$cluster)

sehm(se[,grep("T3|BPA",colnames(se),invert=TRUE)], degs2, do.scale = FALSE, anno_columns = c("Dataset","EXPO2"), toporder = "cluster", anno_rows = "cluster", breaks=0.99,show_rownames = F)
# we isolate clusters that are coherent between the two systems
dc <- list( up.both=cg[[4]],
			down.both=cg[[7]],
			org.only=unlist(cg[c(1,2,3,8)]),
			fetal.only=unlist(cg[c(5,6)])
			)
...
sehm(se[,grep("T3|BPA",colnames(se),invert=TRUE)], asd.degs, do.scale = FALSE, anno_columns = c("EXPO2","Dataset"), toporder = "cluster", show_rownames = TRUE, breaks=0.995, main="ASD-associated consistent DEGs")
```
...
```{r}
load("Data/regulon_weighted.RData")
vi1 <- viper(assays(SEs$chronic.org)$corrected, regulon, verbose=FALSE)
vi2 <- viper(assays(SEs$chronic.fetal)$corrected, regulon, verbose=FALSE)
