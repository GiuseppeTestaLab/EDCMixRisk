...
sehm(se, degs2, do.scale = FALSE, anno_columns = c("Dataset","EXPO2"), toporder = "cluster", anno_rows = "cluster", breaks=0.99,show_rownames = F)

# we isolate clusters that are coherent between the two systems
dc <- list( up.both=unlist(cg[6:8]),
			down.both=cg[[3]],
			discordant=unlist(cg[c(2,5)]),
			fetal.only=cg[[1]])
...
```{r}
asd.ea <- plEA(row.names(se),unlist(dc[1:2]),sets=c(ASD,psychencode))
