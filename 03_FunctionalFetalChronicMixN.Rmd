 ...
 * ASD-related: list containing most relevant ASD or NDD-related genes
 * Metabolism-related
...
load('Data/ASD.RData', verbose=TRUE)
# remove not useful ones. I keep the gene sets in ASD list
rm(SFARI, SFARIgenes, AutismKB, DecipherNeuro, 
   MSSNG, ASDsevere, ID, NeuroOmim, iPSYCH, GDI)

load('Data/MetabGenes.RData', verbose=TRUE)
MetabGeneSet <- metabolism
```
...
Gene ontology enrichment analysis is performed on the set of `r dim(dplyr::filter(Res$table, FDR < FDRTh & abs(logFC) > LogFCTh & logCPM >0))[1]` genes using TopGO with Fisher statistics and weight01 algorithm. The division between up-regulated and down-regulated genes is done on the mean FC between the two treatments. 
...
Gene ontology enrichment analysis is performed on the set of `r dim(dplyr::filter(Res$table, FDR < FDRTh & abs(logFC) > LogFCTh & logCPM >0))[1]` genes using GOSeq approach. On the basis of the analysis settings, GOSeq analysis __`r ifelse(GOSeqEval==TRUE, 'IS', 'IS NOT')` performed__.
...
printTable(cameraWrapper(dea=ResCamera, gsets=NULL, addSets=c(ASD, MetabGeneSet), addDEgenes=TRUE, dea.thres=0.05, minG=5, reportMax=500))
#printTable(cameraWrapper(dea=ResCamera, gsets=c(H1, Kegg, ASD, MetabGeneSet), addSets=NULL, addDEgenes=TRUE, dea.thres=0.05, minG=5, reportMax=500))
```
...
__fgsea analysis: ASD genes__

```{r GseaASD, eval=GseaEval, collapse=TRUE, fig.height=5}
set.seed(42)
ASD$SFARI <- as.character(ASD$SFARI) 
GseaSelASD <- fgsea(FStat, pathways=ASD, nperm=75000, maxSize=500, minSize=10, nproc=1)
data.table::fwrite(GseaSelASD, file=paste0(OutputFolder, '/Gsea/GseaSelASD.txt'), sep="\t", sep2=c("", " ", ""))

GseaResASD <- dplyr::filter(GseaSelASD, padj < 0.1 & abs(NES) > 2) %>% 
  dplyr::mutate(LEGenes=stringr::str_count(leadingEdge, ",")+1) %>% 
  dplyr::mutate(GeneSet='ASD') %>% 
  dplyr::select(1, 2, 3, 5, 9, 10)
GseaResASD[,1]

if(dim(GseaResASD)[1] > 0){
plotGseaTable(ASD[GseaResASD %>% dplyr::arrange(NES) %>% dplyr::pull(pathway)], 
              FStat, GseaSelASD, gseaParam=0.5)
}
```

__fgsea analysis: metabolism-related gene sets __

```{r GseaCellTypes, eval=GseaEval, collapse=TRUE, fig.height=5}
set.seed(42)
GseaSelMet <- fgsea(FStat, pathways=MetabGeneSet, nperm=75000, maxSize=500, minSize=10, nproc=1)
data.table::fwrite(GseaSelMet, file=paste0(OutputFolder, '/Gsea/GseaSelMet.txt'), sep="\t", sep2=c("", " ", ""))

GseaResMet <- dplyr::filter(GseaSelMet, padj < 0.1 & abs(NES) > 2) %>% 
  dplyr::mutate(LEGenes=stringr::str_count(leadingEdge, ",")+1) %>% 
  dplyr::mutate(GeneSet='Metabolism') %>% 
  dplyr::select(1, 2, 3, 5, 9, 10)
GseaResMet[,1]

if(dim(GseaResMet)[1] > 0){
plotGseaTable(CellTypes[GseaResMet %>% dplyr::arrange(NES) %>% dplyr::pull(pathway)], 
              FSat, GseaSelCells, gseaParam=0.5)
}
```
...
```{r}
GseaRes <- rbind(GseaResH1, GseaResKegg, GseaResMet, GseaResASD)
Cols <- c(H1='#e76bf3', Kegg="#39b600", Metabolism="#e58700", ASD='#00b0f6')
