...
#ggsave(Comp$Scatter, filename='Data/ChronicFetal/ScatterV1.pdf', width=10, height=10)
#ggsave(Comp$Scatter, filename='Data/ChronicFetal/ScatterV1.png', width=10, height=10)
```
...
load("Data/MetabGenes.RData", verbose = T)
...
MixN <- list(fetalDEGs=fetal.chronic3, fetalDEGsUp=fetal.chronicUp, fetalDEGsDown=fetal.chronicDown,NonAffectedGenes=controlNeg, random1=random1,random2=random2,random3=random3 )
...
ggsave(dotplot.multintersect.mod(m, th=0.05), filename='Data/ChronicFetal/DotplotFetalNDD.pdf', width=10, height=6)
...
### SFARI genes
```{r, message=FALSE,collapse=TRUE, fig.height=6, fig.width=10}
m <- overlapper::multintersect(ll = MixN, ll2 = SFARIgenes, universe = row.names(DEAs$chronic.fetal),two.tailed = F)
```

```{r, message=FALSE,collapse=TRUE, fig.height=6, fig.width=10}
dotplot.multintersect.mod(m, th=0.05)

ggsave(dotplot.multintersect.mod(m, th=0.05), filename='Data/ChronicFetal/DotplotFetalSFARI.pdf', width=10, height=6)
```

### NPD genes
```{r, message=FALSE,collapse=TRUE, fig.height=6, fig.width=10}
m <- overlapper::multintersect(ll = MixN, ll2 = NeuropsychiatricDiseases, universe = row.names(DEAs$chronic.fetal),two.tailed = F)
```

```{r, message=FALSE,collapse=TRUE, fig.height=6, fig.width=10}
dotplot.multintersect.mod(m, th=0.05)

ggsave(dotplot.multintersect.mod(m, th=0.05), filename='Data/ChronicFetal/DotplotFetalNPD.pdf', width=10, height=6)
```
...
gom.obj <- newGOM(MixN,ASD, genome.size=length(row.names(DEAs$chronic.fetal)))
print(gom.obj)
drawHeatmap(gom.obj)
```
...
### SFARI genes
```{r, message=FALSE,collapse=TRUE, fig.height=6, fig.width=10}
gom.obj <- newGOM(MixN,SFARIgenes, genome.size=length(row.names(DEAs$chronic.fetal)))
#drawHeatmap(gom.obj)
...
### NPD genes
```{r, message=FALSE,collapse=TRUE, fig.height=6, fig.width=10}
gom.obj <- newGOM(MixN,NeuropsychiatricDiseases, genome.size=length(row.names(DEAs$chronic.fetal)))
#drawHeatmap(gom.obj)
#no significant overlap
```
