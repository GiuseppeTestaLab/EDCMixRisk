...
Pairwise mixed-model tests indicate a significant effect for T3, and a bit for 1X. If we group the controls and MixN:
...
```{r, collapse=TRUE}
PVals <- DEAs$chronic.org[intersect(org.chronic, unlist(ProlGenes)),]
...
```{r, collapse=TRUE}
PVals <- DEAs$chronic.org[intersect(org.chronic, unlist(NeuronGenes)),]
PVals <- PVals[complete.cases(PVals), ]
PVals
```
